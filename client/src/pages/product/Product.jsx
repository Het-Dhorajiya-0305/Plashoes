  ...
  useEffect(() => {
    if (productDetail && Array.isArray(productDetail.proSize)) {
      const sizes = productDetail.proSize.map((iteam, index) => (
        <div key={index} className={size===iteam ? "selected-cont" : "not-in"} onClick={() => setSize(iteam)}><p className="size">{iteam}</p></div>
      ));
        ...
        <div className="product-detail-container">
          ...
          <div className="product-detail">
            ...
            <div className="add-to-cart-btn">
              <button onClick={()=>addToCart(size,productDetail)}>add to cart</button>
            </div>
          </div>
        </div>
      </div>
